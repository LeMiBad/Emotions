...
2. Обновляем мастер-ветку `git checkout trunk`, `git pull emotions trunk`, `git rebase trunk emotions/trunk` (могу ошибаться, если будут ошибки - пишете, исправим)
3. Создаем фиче-ветку `git checkout -b <название-тикета-с-ключем>` (например `EMOTION-19: [Календарь] Добавить заметки`)
4. Пишем код
